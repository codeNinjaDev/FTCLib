...
public class Pixy2 {
     ...
     */
    public Pixy2(HardwareMap hw, String deviceName) {
        this.link = new Pixy2I2C();
        init(hw, deviceName);
     ...
     **
     */
    private int init(HardwareMap hw, String deviceName) {
        // Opens link
        int ret = link.open(hw, deviceName);
        if (ret >= 0) {
     ...
     * @return Pixy2 error code
     */
    public byte setLED(int[] color) {
        return setLED(color[0], color[1], color[2]);
    }
