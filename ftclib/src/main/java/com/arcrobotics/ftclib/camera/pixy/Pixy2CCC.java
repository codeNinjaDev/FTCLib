...
public class Pixy2CCC {
     ...
     */
    public int update(boolean wait, int sigmap, int maxBlocks) {
    ...
    public Block getClosestXBlock(int xCoord) {
        int minPixels = Integer.MAX_VALUE;
        Block closestBlock = blocks.get(0);
        for(Block block: blocks) {
            if(Math.abs(xCoord - block.x) < minPixels) {
                minPixels = Math.abs(xCoord - block.x);
                closestBlock = block;
            }
        }
